...
- **Counter certification + graduation rotation — SHIPPED v9.02 (1bcf28c, 2026-09-12), live-tuning** — the stuck counter is now EARNED: a once-a-day **cold check at app open** (up to 5 words said back on a previous day + due re-tests; meaning shown, typed romaji, forgiving normaliser) sets `certLevel`; pass graduates the word out of the sticky batch (that IS the rotation), miss puts it back at the front, no penalty, re-test gaps 3→240 days. Round end shows recalled / warming / stick; Home shows "+N this week" + a door back into a skipped check; "Swap 10/20 anyway" + "Fresh 30" kept. Consequence: existing testers' counters reset to 0 until proven (5/morning). Watch for: tester confusion at the typing step, romaji spellings the normaliser rejects, whether 5/day feels like a wall. Mic proof stays parked (owner: no mic). Copybook counter lines can move from Variant A to Variant B ("the app keeps score") once testers confirm the check feels right.
