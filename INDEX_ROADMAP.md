...
- **Counter certification + graduation rotation — NEXT (owner call 2026-09-12)** — the "words that stick" counter is an honour system today (drill credits correct unless "Missed it"; self-rating after 12 hears) yet it is the guarantee's measuring stick in every ad. Build: (1) **cold check at session open** — ~5 near-stuck words, meaning shown, user types romaji; pass = certified stuck, fail = back to batch; re-tested at growing gaps, a miss un-graduates; (2) **rotation = graduation** — certified words leave the sticky 30-batch, seats top up unseen-first; keep "Swap 10/20 anyway" + "Fresh 30" worded honestly; (3) round-end screen shows recalls / warming / stuck (never 0); (4) home shows "N stuck · +N this week"; mic proof = owner-page option only. Gate: owner taps through the mock first (scratchpad `wordstick-cold-check-mock.html` artifact), then build → headless probe → live. Copy rule meanwhile: no "can't be cheated"/"app keeps score" anywhere (Copybook rev 2 Variant A).
...
- **Reverse course: English for Japanese speakers** — tester suggestion 2026-09-12: the JP↔EN word pairs already exist, so a "learn English" flavor is mostly drill-direction flip + Japanese UI chrome + JP-voice STT/TTS swap. Fits the post-launch languages-in-staging track (after Spanish/Korean, or possibly cheaper than either since no new vocab pack is needed). Untouched until owner sequences it.
...
- **Kanji + furigana default display** (v8.94, 2026-09-07): owner-locked same day on tester feedback (supersedes 23 Aug opt-in-toggle plan; "Show kanji" COMING-SOON row removed). JMdict-scripted `kj:` field on 1,566/2,177 active words (rest are legitimately kana: particles, greetings, uk-tagged, loanwords); `jpDisplay()`/`rubyHTML()` render `<ruby>` at all 32 word display sites; alignment verified at build AND runtime with kana fallback; TTS/STT/SRS still key off kana `jp:`. Headless probe PASS.
