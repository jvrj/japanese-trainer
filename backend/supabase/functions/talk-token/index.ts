...
function instructions(name: string, owned: W[], due: W[], fresh: W[], minutes: number): string {
  ...
  return [
    ...
    `WHEN ASKED TO EXPLAIN ("explain that", "what does that mean", "in English"): give the plain English meaning of the last Japanese you said, in one short line, for example "ありがとう means thank you." Then wait. Do not tell them to repeat it.`,
    `GENERAL QUESTIONS (weather, news, facts, opinions, anything about the world): answer like a helpful friend in ONE short line of English from what you know. If it needs live information you cannot see, say what is usually true instead, for example "I can't see today's weather, but Tokyo is warm in September." Never answer "I do not know". Never build a Japanese sentence out of English words. After answering, gently come back to a ledger word in your NEXT turn, not the same one.`,
...
Deno.serve(async (req) => {
  ...
  const session = {
    ...
    audio: {
      input: {
        transcription: { model: 'gpt-4o-mini-transcribe', prompt: 'こんにちは、げんきですか？ ありがとう。おはようございます。すみません、もういちど おねがいします。 Hello, how do I say thank you?' },  // no forced language: the learner may speak English or Japanese
        turn_detection: { type: 'server_vad', threshold: 0.5, prefix_padding_ms: 300, silence_duration_ms: 500, create_response: true, interrupt_response: true },  // server_vad answers ~0.65 s after the learner stops; semantic_vad/low sat silent on the phone
      },
